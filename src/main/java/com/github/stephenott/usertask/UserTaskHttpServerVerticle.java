        ...
        completeRoute.handler(rc -> {
            ...
            eb.<DbActionResult>request(address, completionRequest, reply -> {
                if (reply.succeeded()) {
                    ...
                    if (reply.result().body().getResultObjects().size() == 1) {
                        rc.response().end(JsonObject.mapFrom(reply.result().body().getResultObjects().get(0)).toBuffer());
                    } else {
    ...
    private void establishGetTasksRoute(Router router) {
        ...
        getRoute.handler(rc -> {
            ...
            eb.<DbActionResult>request(address, getRequest, reply -> {
                if (reply.succeeded()) {
                    addCommonHeaders(rc.response());
                    rc.response().end(new JsonArray(reply.result().body().getResultObjects()).toBuffer());

                } else {
                    Class eClass = reply.cause().getClass();

                    if (eClass.equals(FailedDbActionException.class)) {
...
//            });
    ...
    private Future<Void> completeUserTask(CompletionRequest completionRequest) {
        ...
        eb.<DbActionResult>request(ebAddress, completionRequest, dbCompleteHandler -> {
            if (dbCompleteHandler.succeeded()) {
//               promise.complete(dbCompleteHandler.result().body());
                promise.complete();
            } else {
